 ...
 *
 */
public class Inmuebles extends JFrame {
	...
	private JTable jTableInmuebles = null;
	...
	// rellena el JTable con los inmuebles encontradas
	{
		
		try {
			this.controladorInmueble.GetInmuebles();
		} catch (DAOExcepcion e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			JOptionPane.showMessageDialog(null, "Error en la capa de acceso a datos", "No se puede acceder a Películas", JOptionPane.ERROR_MESSAGE);
	    ...
	    for (int i1 = 0; i1 < controladorInmueble.GetNumInmuebles(); i1++) {
	      i = (Inmueble) controladorInmueble.GetActual(i1);
	      
	      this.jTableInmuebles.getModel().setValueAt(new Integer(i.getId()),i1, 0);
	      this.jTableInmuebles.getModel().setValueAt(new String(i.getEscalera()),i1,1);
	      this.jTableInmuebles.getModel().setValueAt(new String(i.getPiso()),i1,2);
	      this.jTableInmuebles.getModel().setValueAt(new String(i.getPuerta()),i1,3);
	      this.jTableInmuebles.getModel().setValueAt(i.getPorcentaje(),i1,4);
	      this.jTableInmuebles.getModel().setValueAt(i.getPropietario().getNif(),i1,5);
	      this.jTableInmuebles.getModel().setValueAt((i.getComunidad().getIdComunidad()),i1,6);
	    }
	    // selecciona la primera fila
	    if (jTableInmuebles.getRowCount() > 0)
	    	jTableInmuebles.addRowSelectionInterval(0, 0);
	}
	...
	{
		int fila = jTableInmuebles.getSelectedRow();
		if ((0<=fila)&(fila<=jTableInmuebles.getRowCount()-1)) return true;
		else return false;
	}
	...
	private JScrollPane getJScrollPaneTabla() {
		if (jScrollPaneTabla == null) {
			...
			jScrollPaneTabla.setViewportView(getJTableInmuebles());
		}
		return jScrollPaneTabla;
	}

	/**
	 * This method initializes jTableInmuebles	
	 * 	
	 * @return javax.swing.JTable	
	 */
	private JTable getJTableInmuebles() {
		if (jTableInmuebles == null) {
			jTableInmuebles = new JTable();
			jTableInmuebles.setModel(MiModeloTabla);
			// evitar selección multiple
			jTableInmuebles.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);
		    // cambiar el color de la cabecera de la tabla.
		    JTableHeader CabeceraTabla = jTableInmuebles.getTableHeader();
		    CabeceraTabla.setBackground(new Color(216, 228, 248));
		    CabeceraTabla.setForeground(new Color(16, 101, 158));
		}
		return jTableInmuebles;
	}
//////////////////////////////// Modelo de la tabla ///////////////////////
